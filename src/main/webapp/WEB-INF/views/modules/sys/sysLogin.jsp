...
<head>
	...
	<style type="text/css">
      html,body,table{background-color:#f5f5f5;width:100%;text-align:center;}.form-signin-heading{font-family:Helvetica, Georgia, Arial, sans-serif, 黑体;font-size:36px;margin-bottom:20px;color:#51b339;}
      .form-signin{position:relative;text-align:left;width:300px;padding:25px 29px 29px;margin:0 auto 20px;background-color:#fff;border:1px solid #e5e5e5;
        	-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px;-webkit-box-shadow:0 1px 2px rgba(0,0,0,.05);-moz-box-shadow:0 1px 2px rgba(0,0,0,.05);box-shadow:0 1px 2px rgba(0,0,0,.05);}
      .form-signin .checkbox{margin-bottom:10px;color:#51b339;} .form-signin .input-label{font-size:16px;line-height:23px;color:#999;}
	...
	<script type="text/javascript">
		...
		if(self.frameElement && self.frameElement.tagName == "IFRAME" || $('#left').length > 0 || $('.jbox').length > 0){
			alert('未登录、登录超时或者账号已在别处登录。请重新登录，谢谢！');
			top.location = "${ctx}";
		}
	</script>
...
<body>
	...
	<form id="loginForm"  name="loginForm" class="form-signin" action="${ctx}/login" method="post" EnableViewState="false">
		<label class="input-label" for="username">登录名</label>
		<input type="text" id="username" name="username" class="input-block-level required"  onkeyup="value=value.replace(/[\W]/g,'')" maxlength="420" minlength="8" htmlEscape="true"/>
		...
		<input id="nbtnSubmit"  class="btn btn-large btn-primary" type="submit" value="登 录" onclick="Encrypt();"/>&nbsp;&nbsp;
...
function Encrypt(){
	     ...
	     var loginName =document.getElementById("username").value//登录名
