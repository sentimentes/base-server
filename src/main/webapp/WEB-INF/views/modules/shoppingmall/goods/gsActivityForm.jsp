...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/shoppingmall/goods/gsActivity/">商城活动管理列表</a></li>
		<li class="active"><a href="${ctx}/shoppingmall/goods/gsActivity/form?id=${gsActivity.id}">商城活动管理<shiro:hasPermission name="shoppingmall:goods:gsActivity:edit">${not empty gsActivity.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="shoppingmall:goods:gsActivity:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="gsActivity" action="${ctx}/shoppingmall/goods/gsActivity/save" method="post" class="form-horizontal" enctype="multipart/form-data" >
		<form:hidden path="id"/>
		...
		<div class="control-group">
		    ...
		    <c:if test="${not empty gsActivity.image}">
			...
			<div id="myModal" class="modal" style=" width: 50%;height: 50%;margin:0 auto;z-index:9999" >
				 ...
				 <c:set value="${fn:split(gsActivity.image,',')}" var="imgArray" />
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="office" name="office.id" value="${gsActivity.office.id}" labelName="office.name" labelValue="${gsActivity.office.name}"
		...
		<div class="control-group">
				...
				<div class="controls">
					<form:select path="types" id="bindType" class="input-xlarge required"  style="width:285px">
						<form:option value="1" >公告</form:option>
						<form:option value="2" >特惠</form:option>
					</form:select>
					<span class="help-inline"><font color="red">*</font>（必填）请选择广告绑定类型</span>
				</div>
			</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="shoppingmall:goods:gsActivity:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
