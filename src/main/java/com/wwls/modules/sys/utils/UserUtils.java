...
import com.wwls.modules.sys.dao.AreaDao;
...
public class UserUtils {
	...
	private static SysSessionManageService sysSessionManageService = SpringContextHolder.getBean(SysSessionManageService.class);
			...
			if (user.isAdmin()){
				...
				if(roleList!=null && roleList.size()>0){
				    for(Role newRole : roleList){
				    	//role.name=平台运营管理员
					     if(newRole.getId().equals("86e2cc1978044b95b548ff3256696da2")){
					    	 isAdmin = true; 
					    	 break;
					     }
				     }
...
//	}
	 ...
	 * @return 取不到返回null
	 */
	public static User checkCacheLoginName(String loginName){
		User user = (User)CacheUtils.get(USER_CACHE, USER_CACHE_LOGIN_NAME_ + loginName);
		if (user == null){
			return null;
		}
	 ...
	 * @return 取不到返回 new User()
	 */
	public static SysSessionManage getDate(){
		SysSessionManage sysSessionManage = sysSessionManageService.get(""+1);
		if (sysSessionManage!=null){
			
			return sysSessionManage;
		}
		// 如果没有登录，则返回实例化空的User对象。
		return new SysSessionManage();
	}
}
